...
from lazagne.config.write_output import write_header, write_footer, print_footer, print_debug, parseJsonResultToBuffer, \
    print_output
...
def launch_module(module, need_high_privileges=False, need_system_privileges=False, not_need_to_be_in_env=False,
...
# Run only one module
def runModule(category_choosed, need_high_privileges=False, need_system_privileges=False, not_need_to_be_in_env=False,
              cannot_be_impersonate_using_tokens=False):
    global category
    ...
    for categoryName in category:
        for r in launch_module(modules[categoryName], need_high_privileges, need_system_privileges,
                               not_need_to_be_in_env, cannot_be_impersonate_using_tokens):
            yield r
...
def set_env_variables(user=getpass.getuser(), toImpersonate=False):
    ...
    if not toImpersonate:
        ...
        constant.profile['COMPOSER_HOME'] = os.environ.get('COMPOSER_HOME',
                                                           'C:\\Users\\%s\\AppData\\Roaming\\Composer\\' % user)
    else:
        ...
        for user in impersonateUsers:
            ...
            for sid in impersonateUsers[user]:
                try:
                    ...
                    for r in runModule(category_choosed, need_system_privileges=_need_system_privileges,
                                       cannot_be_impersonate_using_tokens=_cannot_be_impersonate_using_tokens):
                        yield r
...
if __name__ == '__main__':
    ...
    PPoptional = argparse.ArgumentParser(add_help=False, formatter_class=lambda prog: argparse.HelpFormatter(prog,
    ...
    # Output
    PWrite = argparse.ArgumentParser(add_help=False, formatter_class=lambda prog: argparse.HelpFormatter(prog,
                                                                                                         max_help_position=constant.MAX_HELP_POSITION))
    ...
    all_subparser = []
    for c in category:
        category[c]['parser'] = argparse.ArgumentParser(add_help=False,
        ...
        for module in modules[c].keys():
            m = modules[c][module]
            category[c]['parser'].add_argument(m.options['command'], action=m.options['action'], dest=m.options['dest'],
                                               help=m.options['help'])
            ...
            if m.suboptions and m.name != 'thunderbird':
                ...
                for sub in m.suboptions:
                    tmp_subparser = argparse.ArgumentParser(add_help=False,
                                                            formatter_class=lambda prog: argparse.HelpFormatter(prog,
                                                                                                                max_help_position=constant.MAX_HELP_POSITION))
